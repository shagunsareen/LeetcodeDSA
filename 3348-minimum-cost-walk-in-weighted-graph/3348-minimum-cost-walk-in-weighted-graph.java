class Solution {
    public int[] minimumCost(int n, int[][] edges, int[][] query) {
        //Step 1 : Initialise disjoint set and adjacency list 
        int[] dsuf = new int[n]; //since we have 0 to n-1 vertices
        Arrays.fill(dsuf, -1);
        List<List<int[]>> adj = new ArrayList<>();
        for(int i=0; i<n; i++){
            adj.add(new ArrayList<>()); //adding list for every node to track adjacent nodes of curr node
        }
        ...
        for(int[] edge : edges){
            ...
            if(px != py){ //if both doesn't have same parent node then make one of it as parent of other
                dsuf[px] = py; //union
            }
        }
    ...
    private void traverseComponent(int[] bitwiseAnd, int curr, boolean[] visited, List<List<int[]>> adj){
        ...
        for(int[] edge : adj.get(curr)){
            ...
            if(!visited[neighbour]){
                traverseComponent(bitwiseAnd, neighbour, visited, adj);
            }
        }
    }
}
